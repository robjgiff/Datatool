...
#***************************************************************************
# Subroutine:  ncbi_to_digs
# Description: Convert NCBI fasta to DIGS fasta
#***************************************************************************
sub ncbi_to_digs {

	my ($self, $seqs_ref, $digs_seq_ref) = @_;

	# Iterate through sequences
	foreach my $sequence_ref (@$seqs_ref) {
		...
		my $element = pop @header;
		my @element = split('\[', $element);
		my $organism = pop @element;
		my $gene = shift @element;
		if ($gene and $organism) {
			...
			$organism =~ s/]//g;
			$organism =~ s/--/-/g;
			#print "\n\t # Element: $gene ($organism";
			
			my $digs_header = ">$organism" . "_$gene";
			print "\n\t # DIGS header: '$digs_header'";
			my $digs_fasta = "$digs_header\n$sequence\n";
		
			push (@$digs_seq_ref, $digs_fasta);
		}
	}
}
...
sub combine_data {
	...
	#$devtools->print_hash(\%seen_in_a); exit;
	...
	foreach my $line (@$file_a) { 
		...
		my @line_bits = split("\t", $line);
		my $id = $line_bits[$a_index];
		#$devtools->print_array(\@line_bits);
